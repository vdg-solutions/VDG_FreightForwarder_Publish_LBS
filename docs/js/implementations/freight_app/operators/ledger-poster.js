...
function buildEntriesFromShipment(shipment, chartOfAccounts, postingRules) {
  const wasm = getWasm();
  const rulesPayload = {
...
function buildEntriesFromCommission(commissionEntry, chartOfAccounts, postingRules) {
  const wasm = getWasm();
  const rulesPayload = {
    pnl_kind_live: postingRules.pnl_kind_live || {},
    commissions: postingRules.commissions || {}
...
function validateEntries(entries, chartOfAccounts) {
 ...
 */
function buildReversalEntry(originalLegs, chartOfAccounts, actorId) {
  const wasm = getWasm();
 ...
 * check below.
 */
async function postReversal(entryId, actorId, ledgerRepo) {
 ...
 */
async function postShipment(shipment, ledgerRepo) {
  if (!ledgerRepo) throw new Error('ledger-poster: ledgerRepo not available');
...
async function postCommission(commissionEntry, ledgerRepo) {
  if (!ledgerRepo) throw new Error('ledger-poster: ledgerRepo not available');
  const [chart, rules] = await Promise.all([ledgerRepo.chartOfAccounts(), ledgerRepo.postingRules()]);
 ...
 */
async function postJournalEntries(entries, ledgerRepo, postedIndex) {
...
/// The operator, bound behind core_abstractions/ports/ledger-poster.js by the freight_app bootstrap.
export const ledgerPoster = { buildEntriesFromShipment, buildEntriesFromCommission, validateEntries, buildReversalEntry, postReversal, postShipment, postCommission, postJournalEntries };

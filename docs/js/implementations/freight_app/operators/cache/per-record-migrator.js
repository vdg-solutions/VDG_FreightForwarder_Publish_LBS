...
async function dedupeByName(ws, trashFile, dir) {
  ...
  if (!listing?.folderId) return 0; // absent folder (server backend answers null, not a fresh one)
  const seen = new Map();
  let trashed = 0;
...
export async function relocateFolders(ws, moveFile, pairs) {
  ...
  for (const { from, to } of pairs) {
    try {
      ...
      if (!src.folderId || !files.length) continue;
...
export async function retireEmptyLegacy(ws, trashFile, paths) {
  ...
  for (const path of paths) {
    ...
    if (!listing?.folderId) continue; // already gone — nothing to retire

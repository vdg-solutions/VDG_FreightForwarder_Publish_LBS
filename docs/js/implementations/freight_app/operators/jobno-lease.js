...
function peekLease(repCode, store = _defaultStore()) {
...
function saveLease(repCode, from, to, store = _defaultStore()) {
  if (!store) return;
...
function takeFromLease(repCode, store = _defaultStore()) {
...
function claimRange(rawContent, repoMaxSeq, blockSize = BLOCK_SIZE) {
  let next = null;
 ...
 */
async function acquireLease(repCode, repoMaxSeq, { blockSize = BLOCK_SIZE, store = _defaultStore() } = {}) {
  try {
    const api = await import('../../storage/core_abstractions/storage-api.js');
    const { getOrCreateFile } = api;
    const { activeWorkspaceName } = await import('../../storage/core_abstractions/workspace-registry.js');
...
/// The operator, bound behind core_abstractions/ports/jobno-lease.js by the freight_app bootstrap.
export const jobnoLease = { peekLease, saveLease, takeFromLease, seedNext, claimRange, acquireLease };

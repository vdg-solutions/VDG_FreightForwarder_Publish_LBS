...
function computeKpis(shipments, lines) {
  const wasm = getWasm();
  const res = wasm.wasm_compute_sales_analytics(JSON.stringify(shipments), JSON.stringify(lines));
  return res.kpis;
}

function computeLeaderboard(shipments, lines) {
  const wasm = getWasm();
  const res = wasm.wasm_compute_sales_analytics(JSON.stringify(shipments), JSON.stringify(lines));
  return res.leaderboard;
}

function computeTopCustomers(shipments, lines, n = 10) {
  const wasm = getWasm();
  const res = wasm.wasm_compute_sales_analytics(JSON.stringify(shipments), JSON.stringify(lines));
...
function computeLaneHeatmap(shipments, lines) {
  const wasm = getWasm();
  const res = wasm.wasm_compute_sales_analytics(JSON.stringify(shipments), JSON.stringify(lines));
  return res.heatmap;
}

function computeMonthlyBars(shipments, lines, months = 12) {
  const wasm = getWasm();
  const res = wasm.wasm_compute_sales_analytics(JSON.stringify(shipments), JSON.stringify(lines));
...
function computeBillingFunnel(shipments) {
  const wasm = getWasm();
...
/// The operator, bound behind core_abstractions/ports/sales-analytics-compute.js by the freight_app bootstrap.
export const salesAnalyticsCompute = { computeKpis, computeLeaderboard, computeTopCustomers, computeLaneHeatmap, computeMonthlyBars, computeBillingFunnel };

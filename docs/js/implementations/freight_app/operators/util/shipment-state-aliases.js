...
import { runSeedMigrations } from '../../core_abstractions/ports/seed-migrator.js';
import { SHIPMENT_STATES_KIND } from '../../core_abstractions/ports/shipment-state-aliases.js';
...
async function ensureShipmentStateAliases(repo) {
  if (!repo) return [];
  await runSeedMigrations(repo, [SHIPMENT_STATES_SEED_MIGRATION]).catch(() => {});
  return repo.list(SHIPMENT_STATES_KIND, null).catch(() => []);
}

/// The operator, bound behind core_abstractions/ports/shipment-state-aliases.js by the freight_app bootstrap.
export const shipmentStateAliases = { ensureShipmentStateAliases };

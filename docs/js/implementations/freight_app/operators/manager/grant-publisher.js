...
import { GRANTS_DIR, DRIVE_ROLE_READER, DRIVE_OP_GRANT_ROLE_FILE } from '../../core_abstractions/ports/grant-publisher.js';
function grantFileOp(userPrefix, op = DRIVE_OP_GRANT_ROLE_FILE, result = 'ok') {
...
async function publishGrant(api, wasm, { rootId, workspace, email, userPrefix, roleToken, areas = null }) {
...
async function unpublishGrant(api, wasm, { rootId, workspace, email, userPrefix }) {
  const name   = wasm.grant_file_target_name(workspace, userPrefix);
  const folder = await api.getOrCreateFolder(rootId, GRANTS_DIR);
...
/// The staff record's role set, tolerating the legacy shape (role + extra_roles) alongside `roles`.
...
/// the record `UserStore` handed us (which is itself the grant, so the diff is against a stale
/// manifest or a legacy shape), and rewrites on difference. `publishGrant` PATCHes in place, so the
/// fileId and the reader permission already on it both survive the rewrite.
///
...
async function backfillGrants(api, wasm, { rootId, workspace, users, resolveAreas = null }) {
...
/// The operator, bound behind core_abstractions/ports/grant-publisher.js by the freight_app bootstrap.
export const grantPublisher = { grantFileOp, publishGrant, unpublishGrant, backfillGrants };

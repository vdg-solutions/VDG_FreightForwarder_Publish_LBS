...
import { takeFromLease, acquireLease } from '../core_abstractions/ports/jobno-lease.js';
...
/** Highest sequence this rep code has already used, as far as the local repo can see. */
export async function repoMaxSeq(repo, repCode) {
  let repoMax = 0;
  try {
  ...
  } catch { /* repo unavailable — 0, the caller's claim carries the mint */ }
  return repoMax;
}
...
export async function nextLocalSeq(repo, repCode) {
  ...
  const repoMax = await repoMaxSeq(repo, repCode);
  if (repoMax >= provisional) {
    // repo already carries a higher seq (another device's write synced in) — jump past it.
    // Past BOTH: a concurrent call may have resumed first and claimed repoMax+1 in the session
    // map already; repoMax+1 alone would hand the same number to every call that awaited over
    // the same repo snapshot.
    const seq = Math.max(repoMax, _sessionSeq.get(repCode) || 0) + 1;
...
// F-41-06 (owner 2026-08-14: every business number mints locally AND cannot collide): the lease
// is the primary mint — a locally-held block claimed by Drive CAS, disjoint per device, so two
// clients can never produce one number. Counted max+1 survives only as the no-lease fallback
// (offline before ever leasing, no ETag to CAS), where the post-write heal (F-41-04) remains
// the backstop.
export async function assignJobNo(repo, repCode) {
  const leased = takeFromLease(repCode);
  if (leased !== null) return formatJobNo(repCode, leased);
  const acquired = await acquireLease(repCode, await repoMaxSeq(repo, repCode)).catch(() => null);
  if (acquired) {
    const seq = takeFromLease(repCode);
    if (seq !== null) return formatJobNo(repCode, seq);
  }

...
import { readRevenue, readRevenueFor, revenuePrefixFor, writeRevenue, deleteRevenue } from '../../core_abstractions/ports/shipment-revenue-repo.js';
import { listWhere } from '../../core_abstractions/repo-query.js';
import { recordShipmentChange } from '../../core_abstractions/ports/shipment-audit.js';
import { assertWritable } from '../../core_abstractions/ports/write-gate.js';


// F-37-02 ops. `state` is separate from `update` because the two are answerable by different
import { KIND_SHIPMENT, OP_SAVE, OP_STATE, OP_DELETE, REVENUE_SEEN } from '../../core_abstractions/ports/shipment-repo.js';
...
async function putShipment(repo, shipment) {
 ...
 */
async function putEnvelope(repo, ref, shipmentLike) {
...
async function getEnvelope(repo, ref) {
  return repo.get(KIND_SHIPMENT, ref);
}

async function listEnvelopes(repo, predicate = null) {
  return listWhere(repo, KIND_SHIPMENT, predicate);
}

async function deleteShipment(repo, ref) {
...
async function getShipment(repo, ref) {
...
async function listShipments(repo, predicate = null) {
...
async function joinLoaded(repo, envelopes) {
  return joinMany(await readRevenueFor(repo, envelopes), envelopes);
}
...
function anyRevenueVisible(rows) {
...
/// The operator, bound behind core_abstractions/ports/shipment-repo.js by the freight_app bootstrap.
export const shipmentRepo = { putShipment, putEnvelope, getEnvelope, listEnvelopes, deleteShipment, getShipment, listShipments, joinLoaded, anyRevenueVisible };

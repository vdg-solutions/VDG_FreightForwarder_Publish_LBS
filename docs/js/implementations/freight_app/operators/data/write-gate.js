...
import { PREF_LOCKED_PERIODS_KEY, PeriodLockedError, LicenseReadOnlyError } from '../../core_abstractions/ports/write-gate.js';
...
async function assertWritable(repo, etd, kind = 'shipment') {
  const lic = (globalThis.window || globalThis).__vdg_license_status;
  if (lic && lic.can_write === false) {
    throw new LicenseReadOnlyError(lic.grace_days_left ?? 0, t('license.readonly_error', { d: lic.grace_days_left ?? 0 }));
  }
  ...
  if (wasm.is_period_closed(kind, BigInt(Math.trunc(etdMs)), JSON.stringify(locked))) {
    const monthKey = new Date(etdMs).toISOString().slice(0, MONTH_KEY_LEN);
    throw new PeriodLockedError(monthKey, t('period.locked_error', { k: monthKey }));
  }
}

/// The operator, bound behind core_abstractions/ports/write-gate.js by the freight_app bootstrap.
export const writeGate = { assertWritable };

...
import { KIND_SHIPMENT_REVENUE } from '../../core_abstractions/ports/shipment-revenue-repo.js';
...
function revenuePrefixFor(shipment) {
 ...
 * surprise.
 */
async function writeRevenue(repo, shipmentRef, revenue, targetPrefix = null) {
...
async function deleteRevenue(repo, shipmentRef) {
 ...
 */
async function readRevenue(repo, shipmentRef, prefix) {
 ...
 */
async function readAllRevenueIn(prefix) {
  const io = ioPort();
  if (!io || !prefix) return [];
...
function clearForkScanCache(prefix = null) {
 ...
 */
async function readRevenueFor(repo, shipments) {
...
/// The operator, bound behind core_abstractions/ports/shipment-revenue-repo.js by the freight_app bootstrap.
export const shipmentRevenueRepo = { revenuePrefixFor, writeRevenue, deleteRevenue, readRevenue, readAllRevenueIn, clearForkScanCache, readRevenueFor };

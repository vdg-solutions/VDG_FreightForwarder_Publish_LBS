...
export function buildFeedHtml(entries) {
  ...
  for (const e of entries) {
    // Joined, not interpolated: this is a grouping key, never text anyone reads, and a template
    // literal over `kind` reads to the i18n gate as an untranslated enum on screen.
  ...
  return items.map((group) => {
    ...
    if (group.length === 1) {
      return `<li class="py-1.5 text-xs text-slate-600 border-b border-slate-50">
        ${relTime(first.ts)} — ${label} · ${first.op || '?'}
      </li>`;
    }
    return `<li class="py-1.5 text-xs border-b border-slate-50">
      <details>
        <summary class="cursor-pointer text-slate-600">${relTime(first.ts)} — ${label} · ${first.op || '?'}</summary>

...
export class UserDriveRepo extends UserRepoPort {
  ...
  /// AC-04: bootstrap — seed admin/users.jsonl with the signing-in manager iff still empty.
  async ensureSeeded(currentUser) {
    await this._repo().users_ensure_seeded(currentUser.email, currentUser.name || '');
  }
}

...
// Cache-busted at build time: 32921e4e is replaced by build_dist.ps1 with the git commit hash.
// Dynamic import bypasses SW stale cache — static import with ?v= query is not valid ESM.
const WASM_URL = new URL('../../../../../pkg/vdg_freight.js?v=32921e4e', import.meta.url).href;
...
function runOp(m) {
  const { store_get, store_list, store_put, store_delete,
          ...
          store_intent_list, store_intent_pending_pack, store_intent_commit, store_intent_parked_list } = _mod;
  switch (m.op) {
    ...
    case 'intentParkedList':  return store_intent_parked_list();

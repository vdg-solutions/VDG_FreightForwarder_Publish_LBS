...
function _requestOwnership() {
  ...
  // The answer is settled AT THE GRANT, inside the callback — never off the request promise.
  // `request` resolves with what the callback returned, and only once that returned promise
  // settles; _holdForever's never does, by design. So on the success path the request promise is
  // pending for the document's whole life, and a boot that waited on it would wait forever: the
  // tab that WINS renders nothing, while the tab that is refused settles on the abort below and
  // looks perfectly healthy. That asymmetry is the whole of the 2026-09-17 outage.
  navigator.locks.request(TAB_OWNER_LOCK, { signal: abort.signal }, () => { settle(true); return _holdForever(); })
    .catch((err) => {
...
/// Resolves once THIS document owns the workspace, or false if the steal did not land — never
/// pending, so the caller can always re-run the boot gate.
export async function takeTabOwnership() {
  ...
  // so resolve on the callback itself rather than assuming it has already run. The catch settles
  // too: a request that rejects BEFORE its callback runs never grants, and a promise nobody
  // settles would wedge the one button that gets a blocked person out of the blocked screen.
  const taken = await new Promise((settle) => {
    navigator.locks.request(TAB_OWNER_LOCK, { steal: true }, () => { settle(true); return _holdForever(); })
      .catch(() => {
        // _held means this document HAD the lock and something stole it back — stand down, the
        // reload lands on the gate. Either way this steal did not leave us owning the workspace.
        if (_held) _yieldToNewOwner();
        settle(false);
      });
  });
  _claim = Promise.resolve(taken);
  return taken;
}

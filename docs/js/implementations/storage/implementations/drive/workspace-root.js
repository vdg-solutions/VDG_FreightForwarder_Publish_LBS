...
import { driveFetch, findFolder } from '../../core_abstractions/storage-api.js';
...
// The staff store, mirrored from boundary::grant_file — used to tell a
// provisioned workspace from an empty same-name shell.
const GRANTS_FOLDER_NAME = 'grants';
const GRANT_NAME_PREFIX  = 'grant.';
...
const _rootCache = new Map(); // name → Promise<{id, ownedByMe}|null>
function _resolveRoot(name) {
  if (!name) return Promise.resolve(null);
  const cached = _rootCache.get(name);
  if (cached) return cached;
  const p = _resolveWorkspaceRoot(name).then(
    (hit) => { if (!hit) _rootCache.delete(name); return hit; },
...
function findWorkspaceRoot(name) {
  return _resolveRoot(name).then((hit) => hit?.id ?? null);
}

/// Does the signed-in account OWN the workspace root? The one claim about authority an invitee
/// cannot manufacture, and the only thing that may confer MANAGER without a grant.
///
/// Seeing the root is not it — sharing the root makes it visible to every invitee, and reading
/// MANAGER off that visibility is the QC-2026-08-09 bug ("ai cũng là manager"). Resolving a root
/// id is not it either: a tenant build verifies its pinned id, which succeeds for anyone who can
/// read the folder.
///
/// No extra Drive call: resolution already knows. The owner search is literally `'me' in owners`,
/// the sharedWithMe fallback is by definition not owned, and the pinned/tenant path asks Drive for
/// `ownedByMe` in the same files.get it already makes. Errors propagate — a transient failure is
/// not "you are not the owner", and answering false there would demote the real owner to
/// /pending-access.
function ownsWorkspaceRoot(name) {
  return _resolveRoot(name).then((hit) => hit?.ownedByMe === true);
}
...
function resetWorkspaceRootCache() { _rootCache.clear(); }
...
/// `ownedByMe` rides along on the verify that already happens — Drive computes it from the ACL,
/// so it is true for exactly one account. Omitted by Drive means not owned; never assume yes.
async function _verifyPinnedRoot(id) {
  const res = await driveFetch('GET', `/files/${id}?fields=id,trashed,ownedByMe`);
  return res && res.trashed !== true ? { id, ownedByMe: res.ownedByMe === true } : null;
}
...
async function _resolveWorkspaceRoot(name) {
  ...
  // `'me' in owners` — anything this query returns is owned by the signed-in account by
  // construction, so the winner needs no second look.
  ...
  if (winner) { _writeRootPin(name, winner.id); return { id: winner.id, ownedByMe: true }; }

  // AC-01/AC-03: not owned by the signed-in user — check whether it was shared instead. Shared is
  // the employee shape: readable, never owned.
  const shared = await findSharedSubfolder(name);
  if (!shared) return null;
  _writeRootPin(name, shared);
  return { id: shared, ownedByMe: false };
}
...
// cleanup stays with dedupeGlobalOwnerFolders, which only deletes what it can actually classify.
async function _pickSeededRoot(found) {
  const seeded = [];
  for (const f of found) {
    try {
      const grants = await findFolder(f.id, GRANTS_FOLDER_NAME);
      if (!grants) continue;
      const q = `name contains '${GRANT_NAME_PREFIX}' and '${grants.id}' in parents and trashed=false`;
...
async function findSharedSubfolder(name) {
  if (!name) return null;
...
async function findSharedFilesByNamePrefix(base) {
  if (!base) return [];
...
/// The workspace-root walk the storage bootstrap binds behind the storage-api port.
export const workspaceRoot = { findWorkspaceRoot, ownsWorkspaceRoot, resetWorkspaceRootCache, findSharedSubfolder, findSharedFilesByNamePrefix, listChildFolder, renameFolder };

...
import { parseJsonlBundle, serializeJsonlBundle } from '../../../kernel/core_abstractions/util/jsonl-bundle.js';
import { DriveApiError, ConcurrencyError } from '../../core_abstractions/drive-errors.js';
import { classifyDriveError, DRIVE_ERROR_KIND_SCOPE_INSUFFICIENT, DRIVE_REASON_SCOPE_INSUFFICIENT } from '../../core_abstractions/drive-error-classifier.js';
...
async function getOrCreateFolder(parentId, name, _opts = {}) {
  _checkForceScope403();
...
async function uploadFile(parentOrFileId, name, content, etag = null, { isUpdate = false } = {}) {
  if (isUpdate || etag) {
...
async function findWorkspaceRoot(name) {
  if (!name) return null;

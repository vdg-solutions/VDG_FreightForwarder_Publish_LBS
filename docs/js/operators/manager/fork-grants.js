 ...
 */
export function splitInteriorWildcard(path) {
...
export async function grantAcrossForks(api, resolvePath, rootId, email, entry) {
...
export async function revokeAcrossForks(api, resolvePath, rootId, email, entry) {
  const parts = splitInteriorWildcard(entry.path);
  if (!parts) return;
  const parentId = await resolvePath(api, rootId, parts.parent);
 ...
 */
export async function grantNewForkToReaders(api, resolvePath, resolveAcl, users, rootId, newPrefix) {
  if (!newPrefix) return [];

...
function genPricingKey(carrier, origin, dest) {
  return `${KIND_PREFIX}-${carrier}-${origin}-${dest}`;
}
...
function displayRate(amount, currency) {
  ...
  return wasm.pnl_round_for_display(Number(amount), currency);
}
...
function openModal(root, entity, onSave) {
  ...
  dialog.querySelector("#ot-form").addEventListener("submit", async (ev) => {
    ...
    setErr("#ot-err-carrier", "");
    setErr("#ot-err-rate", "");
    setErr("#ot-err-dates", "");
    const wasm = window.__vdg_wasm;
    if (!wasm.validate_scac(carrier)) {
      setErr("#ot-err-carrier", "2-4 uppercase letters, e.g. WHLC");
      return;
    }
    if (!Number.isFinite(rate) || rate <= 0) {
      setErr("#ot-err-rate", t("masters.ocean_tariff.err_rate_positive"));
      return;
    }
    if (!wasm.validate_date_range(validFrom, validTo)) {
      setErr("#ot-err-dates", t("masters.ocean_tariff.err_dates_invalid"));
      return;
    }
  ...
  root.querySelector("#m-tbody")?.addEventListener("click", async (ev) => {
    ...
    if (delBtn) {
      const ok = await showConfirm({
        title: t("masters.ocean_tariff.delete_confirm"),

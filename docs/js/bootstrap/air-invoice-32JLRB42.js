...
var CSV_HEADER = "hawb_no,awb_no,mawb_no,flight_no,origin_iata,dest_iata,weight_chargeable_kg,freight,currency,cass_code,cass_eligible";
var MOCK_INVOICES = [
  {
    carrier_iata: "VN",
...
function buildCassCSV(invoices) {
...
function eligibleBadge(inv, idx) {
  ...
  const cls = on ? "bg-green-100 text-green-700 border-green-300" : "bg-slate-100 text-slate-500 border-slate-300";
...
async function render(root) {
  ...
  body.addEventListener("click", (ev) => {
    ...
    btn.className = btn.className.replace(/bg-\S+ text-\S+ border-\S+/g, "").trim() + (on ? " bg-green-100 text-green-700 border-green-300" : " bg-slate-100 text-slate-500 border-slate-300");
  });

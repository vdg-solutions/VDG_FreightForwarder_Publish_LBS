...
async function handleActions(e, root, repo) {
  ...
  if (btn.classList.contains("btn-send")) {
    ...
    root.querySelector(`tr[data-qid="${id}"] .converted-cell`).innerHTML = actionCell(updated, effectiveState(updated), hasRole(ROLE_MANAGER));
    root.querySelector(`tr[data-qid="${id}"] td:nth-child(5)`).innerHTML = stateBadge(effectiveState(updated));
  }

...
function jumpToUnbalancedEntry(entryId, totals) {
  ...
  dlg.addEventListener("click", (ev) => {
    if (ev.target.closest?.("[data-dialog-close]")) dlg.close();
  });
...
function renderUnbalancedList(root, ids) {
...
async function loadOpeningBalance(dataRepo, accountCode, dateFrom, live) {
...
var ACCOUNT_BUTTON_CLASS = "w-full text-left px-2 py-1.5 text-xs rounded hover:bg-slate-100";
...
async function loadLegs(root, refresh) {
  ...
  const res = await safeMasterLoad(
    () => ledgerLegs(_selectedCode, { ..._filter, refresh }),
    LEGS_TAG,
    VIEW_DATA_LOAD_BUDGET_MS
  );
  ...
  if (refresh) {
    const openRes = await safeMasterLoad(
      () => loadOpeningBalance(window.__vdg_repo, _selectedCode, _filter.dateFrom, res.value.opening),
      OPENING_TAG
    );
...
async function loadInitial(root, repo) {
  ...
  if (isViewSuperseded(root)) return;
  if (!chartRes.ok || !chartRes.value.ok) {
    const tree = root.querySelector("#chart-tree");

...
var LOAD_ERROR_MSG = "Couldn't load customers. Please retry.";
var LOAD_RETRY_LABEL = "Retry";
function escHtml(s) {
...
async function render(root) {
  ...
  function renderToolbar(total) {
    const mergeBtn = isM ? `<button id="btn-merge" disabled class="px-3 py-1.5 text-xs rounded bg-amber-100 text-amber-700 hover:bg-amber-200 disabled:opacity-40 disabled:cursor-not-allowed">${t("masters_customers.action.merge")}</button>` : "";
    const addBtn = isM ? `<button id="btn-add" class="text-xs px-3 py-1.5 bg-slate-900 text-white rounded-md hover:bg-slate-800">${t("masters_customers.action.add")}</button>` : "";
    return `

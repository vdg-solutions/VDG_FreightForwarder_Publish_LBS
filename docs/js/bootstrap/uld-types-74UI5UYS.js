...
// output/web/js.tmp/implementations/kernel/core_abstractions/util/uld-validators.js
var ULD_CODE_RE = /^[A-Z]{3}$/;
var ERR_ULD_CODE = "3 uppercase letters, e.g. AKE";
function validateUldCode(code) {
  return ULD_CODE_RE.test(code) ? null : ERR_ULD_CODE;
}
function checkUldCodeUnique(items, code, skipId = null) {
  const dup = items.find((i) => i.code === code && i.id !== skipId);
  return dup ? `ULD code ${code} already exists` : null;
}

// output/web/js.tmp/implementations/ui/bootstrap/views/manager/masters/uld-types.js
var KIND = "uld-types";
var KIND_PREFIX = "ULD";
var SEED_URL = "seed/masters/uld-types.jsonl";
function escHtml(s) {
  return String(s ?? "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
}
function genId(code) {
  return `${KIND_PREFIX}-${code || Date.now()}`;
}
  ...
  return `
    <dialog id="master-modal" class="rounded-xl border border-slate-200 shadow-xl p-0 w-full max-w-md backdrop:bg-black/30">
      <form id="modal-form" method="dialog" class="p-6 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-xs font-medium text-slate-700 mb-1">Volume (m\xB3)</label>
...
function openModal(root, entity, items, onSave) {
  ...
  dialog.querySelector("#modal-form").addEventListener("submit", async (ev) => {
    ...
    if (codeErr) {
      setErr("#m-err-code", codeErr);
      return;
    }
    const dupErr = checkUldCodeUnique(items, code, entity?.id);
    if (dupErr) {
      setErr("#m-err-code", dupErr);
      return;
    }
    if (mgw <= tare) {
      setErr("#m-err-mgw", "MGW must exceed tare");
      return;
    }
        ...
        <table class="w-full">
          <thead class="bg-slate-50 text-[10px] uppercase tracking-wide text-slate-500">
            <tr>
              ...
              <th class="px-3 py-2 text-right">Volume (m\xB3)</th>
  ...
  root.querySelector("#m-tbody")?.addEventListener("click", async (ev) => {
    ...
    if (delBtn) {
      const ok = await showConfirm({
        title: t("uld_types.confirm_delete"),

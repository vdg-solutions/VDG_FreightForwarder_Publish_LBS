...
// output/web/js.tmp/implementations/ui/bootstrap/views/manager/masters/airline-carriers.js
var KIND = "airline-carriers";
var KIND_PREFIX = "ACR";
var SEED_URL = "seed/masters/airline-carriers.jsonl";
function escHtml(s) {
...
function openModal(root, entity, items, onSave) {
  ...
  dialog.querySelector("#modal-form").addEventListener("submit", async (e) => {
    ...
    if (iataErr) {
      setErr("#m-err-iata", iataErr);
      return;
    }
    const icaoErr = validateAirlineIcao(icao);
    if (icaoErr) {
      setErr("#m-err-icao", icaoErr);
      return;
    }
    if (!name) {
      setErr("#m-err-name", "Name is required");
      return;
    }
    const dupErr = checkIataUnique(items, iata, entity?.id);
    if (dupErr) {
      setErr("#m-err-iata", dupErr);
      return;
    }
...
async function render(root) {
  ...
  async function reload() {
    ...
    const listRes = await boundedList(repo, KIND, "airline-carriers:list");
    if (!listRes.ok) {
      if (tbody) tbody.innerHTML = "";
    ...
    items = listRes.value;
    if (isM) items = await boundedSeedIfEmpty(repo, KIND, SEED_URL, items, (e) => genId(e.iata_code), "airline-carriers:seed");
  ...
  root.querySelector("#m-tbody")?.addEventListener("click", async (ev) => {
    ...
    if (delBtn) {
      const ok = await showConfirm({
        title: t("airline_carriers.confirm_delete"),

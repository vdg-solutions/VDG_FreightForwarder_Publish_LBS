...
import {
  showConfirm
} from "./chunk-37E2XNRM.js";
import "./chunk-PVBCNCFR.js";
import {
  t
} from "./chunk-G2RKYR7P.js";

// output/web/js.tmp/implementations/ui/bootstrap/components/approval-card.js
import { LitElement, html } from "https://cdn.jsdelivr.net/npm/lit@3.1.4/+esm";
var SWIPE_THRESHOLD_PX = 60;
var APPROVAL_TYPE_COLORS = {
...
var APPROVAL_SLA_HOURS = 24;
var ANIMATE_OUT_MS = 300;
var JUST_NOW_MAX_MINS = 1;
...
var VdgApprovalCard = class extends LitElement {
  static properties = {
    item: { type: Object },
    ...
    _swipeHint: { type: String, state: true }
    // 'approve' | 'reject' | null
  };
  createRenderRoot() {
    return this;
  ...
  async _loadReps() {
    ...
    try {
      ...
      console.error("[approval-card] rep load failed:", err);
    }
  }
  _onTouchStart(e) {
  ...
  _submit() {
    ...
    const decision = {
      ...
      delegated_to: action === "Delegate" ? this._delegateTo : void 0
    };
    if (action !== "NeedInfo") {
      this._animateOut(() => {
  ...
  _renderActions() {
    ...
    if (this._action === "Delegate") {
      return html`
        <div class="mt-3 flex items-center gap-2">
          <select @change="${(e) => {
        this._delegateTo = e.target.value;
      }}"
    ...
    const btnCls = this._action === "Approve" ? "bg-emerald-600 hover:bg-emerald-700" : this._action === "Rejected" ? "bg-red-600 hover:bg-red-700" : "bg-slate-600 hover:bg-slate-700";
    return html`
  ...
  render() {
    ...
    const swipeCls = this._swipeHint === "approve" ? "ring-2 ring-emerald-400 bg-emerald-50/30" : this._swipeHint === "reject" ? "ring-2 ring-red-400 bg-red-50/30" : "";
    return html`
      <div
...
function applyFilter(items) {
  return items.filter((a) => {
    ...
    if (_filter.urgent && ageHours2(a.requested_at) <= APPROVAL_SLA_HOURS) return false;
    return true;
  });
}
...
function renderCards(root, items) {
  ...
  list.innerHTML = filtered.length === 0 ? `<div class="text-center text-slate-400 text-sm py-12">${t("approvals.empty")}</div>` : "";
...
async function render(root) {
  ...
  root.addEventListener("vdg:approval-decision", async (e) => {
    ...
    try {
      ...
      console.warn("[approvals] write failed:", err.message);
    }
  });
  _onEntity = async (e) => {
    const { kind } = e.detail || {};

...
// output/web/js.tmp/implementations/kernel/core_abstractions/util/uom-validators.js
var ERR_CODE_REQ = "uom.err.code_required";
var ERR_LABEL_REQ = "uom.err.label_required";
function genUnitId(code) {
  return String(code || "").trim().toUpperCase();
}
function validateUnit(code, labelVi) {
  if (!code || !code.trim()) return ERR_CODE_REQ;
  if (!labelVi || !labelVi.trim()) return ERR_LABEL_REQ;
  return null;
}
function checkCodeUnique(items, code, skipId = null) {
...
function openModal(root, entity, items, onSave) {
  ...
  dialog.querySelector("#uom-modal-form").addEventListener("submit", async (ev) => {
    ...
    if (errKey) {
      setErr(errKey === "uom.err.code_required" ? "#m-err-code" : "#m-err-label-vi", t(errKey));
      return;
    }
    if (!entity) {
...
async function loadUnits(repo) {
  return safeMasterLoad(async () => {
    await runSeedMigrations(repo, [SEED_MIGRATION]);

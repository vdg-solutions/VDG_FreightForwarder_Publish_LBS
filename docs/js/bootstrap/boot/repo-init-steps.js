...
export async function runRepoInitBounded(user, stepRef, bootFn, existingDb, onDbOpen) {
  ...
  const wasmMod = await import(new URL('pkg/vdg_freight.js?v=66e0f5f', document.baseURI).href);
  const wasmUrl = new URL('pkg/vdg_freight_bg.wasm?v=66e0f5f', document.baseURI).href;

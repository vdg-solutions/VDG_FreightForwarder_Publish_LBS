...
export async function runRepoInitBounded(user, stepRef, bootFn, existingDb, onDbOpen) {
  ...
  const wasmMod = await import(new URL('pkg/vdg_freight.js?v=7f9d5ab', document.baseURI).href);
  const wasmUrl = new URL('pkg/vdg_freight_bg.wasm?v=7f9d5ab', document.baseURI).href;

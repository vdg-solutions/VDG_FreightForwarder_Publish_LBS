...
export async function runRepoInitBounded(user, stepRef, bootFn, existingDb, onDbOpen) {
  ...
  const wasmMod = await import(new URL('pkg/vdg_freight.js?v=b8569cd', document.baseURI).href);
  const wasmUrl = new URL('pkg/vdg_freight_bg.wasm?v=b8569cd', document.baseURI).href;

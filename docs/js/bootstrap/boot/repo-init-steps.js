...
export async function runRepoInitBounded(user, stepRef, bootFn, existingDb, onDbOpen) {
  ...
  const wasmMod = await import(new URL('pkg/vdg_freight.js?v=eea91f4', document.baseURI).href);
  const wasmUrl = new URL('pkg/vdg_freight_bg.wasm?v=eea91f4', document.baseURI).href;

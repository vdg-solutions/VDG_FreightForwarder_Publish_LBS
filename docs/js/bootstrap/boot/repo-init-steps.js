...
export async function runRepoInitBounded(user, stepRef, bootFn, existingDb, onDbOpen) {
  ...
  const wasmMod = await import(new URL('pkg/vdg_freight.js?v=4d7b136', document.baseURI).href);
  const wasmUrl = new URL('pkg/vdg_freight_bg.wasm?v=4d7b136', document.baseURI).href;

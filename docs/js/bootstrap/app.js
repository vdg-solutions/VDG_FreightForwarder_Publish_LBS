  ...
  _renderNav() {
    return html`
      ...
      <div class="mt-auto px-4 py-3 border-t border-slate-800 text-[10px] text-slate-500 flex items-center justify-between">
        ...
        <span class="font-mono whitespace-nowrap" title="build 98da13ec">v0.4.102 (98da13ec)</span>
      </div>
    `;
  }
  render() {
    ...
    <div class="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-2xl shadow-lg border border-slate-100 w-full max-w-sm p-10 flex flex-col items-center gap-6">
        ...
        <div class="text-[10px] text-slate-300 text-center">
          ${t("login.footer")}
          <div class="mt-1 font-mono text-slate-400">v0.4.102 (98da13ec)</div>
        </div>
      </div>
    </div>`;
}
...
function _requestOwnership() {
  ...
  navigator.locks.request(TAB_OWNER_LOCK, { signal: abort.signal }, () => {
    settle(true);
    return _holdForever();
  }).catch((err) => {
    if (_held) {
...
var TAB_OWNERSHIP_ANSWER_DEADLINE_MS = 5e3;
...
async function answerTabOwnership(claim = claimTabOwnership()) {
...
function loadOnce() {
  ...
  if (!inflight) {
    inflight = (async () => {
      const mod = await import(new URL("pkg/vdg_freight.js?v=98da13ec", document.baseURI).href);
      const wasmUrl = new URL("pkg/vdg_freight_bg.wasm?v=98da13ec", document.baseURI).href;
...
function composeAuth(wasm4) {
  const sessionRoles = {
    currentAccount: () => wasm4.auth_session_roles({}).account ?? null,
    currentRoleToken: () => wasm4.auth_session_roles({}).token ?? null,
...
var wasmShipmentMode = {
  resolveMode: (stored) => {
    ...
    if (typeof mod?.shipment_mode_resolve !== "function") {
      console.warn("[shipment-mode] wasm export missing \u2014 mode left unread");
...
var APP_VERSION = "v0.4.102 (98da13ec)";
...
function bindSalesData({ wasm: wasm4 }) {
  ...
  bindSalesReads({
    ...
    jobHasNoCosts: (lines) => wasm4.sales_job_has_no_costs(lines ?? []),
...
function composeData(wasm4) {
  ...
  bindShipmentRepo({
    ...
    // this layer applies.
    listShipments: async (repo3) => {
      const reply = await wasm4.data_list_envelopes({ mine: false });
      if (!reply.ok) throw new Error(reply.error || "the read failed");
      return joinLoaded(repo3, reply.rows);
    },
    listMyShipments: async (repo3) => {
      const reply = await wasm4.data_list_envelopes({ mine: true });
      if (!reply.ok) throw new Error(reply.error || "the read failed");
      return joinLoaded(repo3, reply.rows);
    },
    joinLoaded,
...
function composeFlows(wasm4) {
  bindSalesRepDerivation({
    ...
    selfRepCandidate: (roles, account) => wasm4.flows_self_rep_candidate({ roles: roles || [], account: account ?? null }).rep,
    mineOnly: (rows) => wasm4.flows_mine_only({ rows: rows || [] }).rows,

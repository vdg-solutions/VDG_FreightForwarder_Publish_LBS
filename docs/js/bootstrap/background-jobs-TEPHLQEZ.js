...
// output/web/js.tmp/implementations/ui/bootstrap/views/background-jobs.js
import { LitElement, html } from "https://cdn.jsdelivr.net/npm/lit@3.1.4/+esm";
var BackgroundJobsView = class extends LitElement {
  static properties = {
  ...
  connectedCallback() {
    ...
    this._unsubscribe = jobTracker.subscribe((jobs) => {
      this._jobs = jobs;
    });
  ...
  render() {
    return html`
      ...
      const isCyclic = job.nextRunAt !== void 0;
      return html`
                <div class="bg-white border ${isError ? "border-red-200" : "border-slate-200"} rounded-xl p-5 shadow-sm transition-all hover:shadow-md">
                  ...
                  <div class="w-full bg-slate-100 rounded-full h-3 overflow-hidden shadow-inner">
                    <div class="h-full rounded-full transition-all duration-500 ease-out flex items-center justify-center 
                      ${isError ? "bg-red-500" : isDone ? "bg-indigo-500" : "bg-gradient-to-r from-blue-400 via-indigo-500 to-purple-500 bg-[length:200%_auto] animate-[gradient_2s_linear_infinite]"}" 
                      style="width: ${isDone || isError || job.progress === void 0 ? "100%" : job.progress + "%"}">
                    </div>
                  </div>
                </div>
              `;

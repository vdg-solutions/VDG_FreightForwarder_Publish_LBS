  ...
  return `
    <dialog id="master-modal" class="rounded-xl border border-slate-200 shadow-xl p-0 w-full max-w-md backdrop:bg-black/30">
      <form id="modal-form" method="dialog" class="p-6 space-y-4">
        <div class="text-base font-semibold text-slate-900 mb-1">${entity ? 'Edit Service' : 'New Service'}</div>
        <div>
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div>
            <label class="block text-xs font-medium text-slate-700 mb-1">Default Kind</label>
  ...
  root.innerHTML = `
    <div class="p-6 max-w-[1100px] mx-auto">
      <div class="flex items-center justify-between mb-6">
        <div class="text-lg font-semibold text-slate-900">Services</div>
        ...
        <table class="w-full">
          <thead class="bg-slate-50 text-[10px] uppercase tracking-wide text-slate-500">
            <tr>
              ...
              <th class="px-3 py-2 text-left">Default Kind</th>
              ${actCol}
            </tr>
          </thead>
          <tbody id="m-tbody"></tbody>
        </table>
        <div id="m-empty" class="hidden text-center text-xs text-slate-400 py-8">No services found.</div>
      </div>
  ...
  root.querySelector('#m-tbody')?.addEventListener('click', async (e) => {
    ...
    if (delBtn) {
      const ok = await showConfirm({
        title: 'Delete this service?', confirmLabel: 'Delete', cancelLabel: 'Cancel', destructive: true,
      });
      if (!ok) return;
